...
class _QuizPageState extends State<QuizPage> {
  ...
  Future<void> fetchQuestions() async {
    ...
    _quizQuestions = new QuestionList(
      results.toList()
    );
  ...
  void choiceHandler(String choice) {
    ...
    if(_questionNumber == _quizQuestions.length) {
      Timer(Duration(seconds: 3), () => Navigator.of(context).push(new MaterialPageRoute(builder: (BuildContext context) => ScorePage(_score, _quizQuestions.length))));
    }
      print(choice);
    ...
    return Column(
      children: <Widget>[
        ...
        Padding(
          ...
          child: Row(
            children: <Widget>[
              new QuestionBox(Colors.red, "A", obj.allAnswers[0], () {choiceHandler(obj.allAnswers[0]);}, _isButtonDisabled),
              new QuestionBox(Colors.red, "B", obj.allAnswers[1], () {choiceHandler(obj.allAnswers[1]);}, _isButtonDisabled)
            ],
          ),
        ),
        Padding(
          ...
          Row(
            children: <Widget>[
              new QuestionBox(Colors.red, "C", obj.allAnswers[2], () {choiceHandler(obj.allAnswers[2]);}, _isButtonDisabled),
              new QuestionBox(Colors.red, "D", obj.allAnswers[3], () {choiceHandler(obj.allAnswers[3]);}, _isButtonDisabled)
            ],
          ) : Container(),
        ),
      ]
            ...
            SingleChildScrollView(
              child: Column(
                children: <Widget>[
                  Center(
                    child: Container(
                      ...
                      padding: new EdgeInsets.only(top: 30.0, bottom: 10.0),
                      child: Center(
                        child: Text("QUESTION " + _questionNumber.toString(),
                        style: new TextStyle(
                          color: Colors.white,
                          fontSize: 25.0,
                ...
                Padding(
                  ...
                  child: _overlayVisible ? new CustomOverlay(_isCorrect) : null,
                ),
